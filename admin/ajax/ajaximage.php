...
if (isset($_SESSION['aid'])) {
    ...
    if (verify_token($website_url.'admin/general', $name = $id_csrf)) {
        ...
        if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == 'POST') {
            $name = htmlentities($_FILES['photoimg']['name']);
            $size = htmlentities($_FILES['photoimg']['size']);

            if (strlen($name)) {
                list($txt, $ext) = explode('.', $name);
                if (in_array($ext, $valid_formats)) {
                    $actual_image_name = time().substr(str_replace(' ', '_', $txt), 5).'.'.$ext;
                    $tmp = htmlentities($_FILES['photoimg']['tmp_name']);
                    if (move_uploaded_file($tmp, $path.$actual_image_name)) {
                        update_back_img1($path_db.$actual_image_name);
