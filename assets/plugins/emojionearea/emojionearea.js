...
; ( function ( factory, global ) {
    ...
    function divis(prefix) {
        var parent = $('<i/>', isObject(prefix) ? prefix : {"class" : selector(prefix, true)});
        $.each(slice.call(arguments).slice(1), function(i, child) {
            if ($.isFunction(child)) {
                child = child.call(parent);
            }
...
// see https://github.com/Modernizr/Modernizr/blob/master/feature-detects/storage/localstorage.js
    ...
    function init(self, source, options) {
        ...
        var id_text = source.attr("id");
        var id_text = id_text+"_id";

        var pid_text = source.attr("pid");

        var sourceValFunc = source.is("TEXTAREA") || source.is("INPUT") ? "val" : "text",
            editor, button, camera, picker, tones, filters, filtersBtns, search, emojisList, categories, scrollArea,
            app = div({
            ...
            button = self.button = divis(' emojionearea-button fa fa-camera camera-com').attr({
                placeholder: options.placeholder || source.data("placeholder") || source.attr("placeholder") || "",
                tabindex: 0,
                pid:  pid_text || "",
                tabindex: 0
            }),
           /* camera = self.camera = div('camera',
                divis(' fa fa-camera camera-com')
            ).attr('title', 'Upload a photo'),*/
